...
export default function DashboardPage() {
  ...
  const handleAccept = async (id: number) => {
      if(confirm("Bạn có chắc muốn nhận yêu cầu này? Trạng thái sẽ chuyển sang ĐANG XỬ LÝ.")) {
          try {
  ...
  const columns: Column<Issue>[] = [
    ...
    { 
        ...
        render: (i) => (
            <div>
                <p className="font-medium text-sm text-slate-900">{i.reporter?.name || i.reporterName || "Unknown"}</p>
    ...
    { 
        ...
        render: (i) => i.assignee ? (
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs">{i.assignee.name}</span>
            </div>
    ...
    },
    {
        ...
        render: (issue) => (
            <div className="flex justify-end gap-2">
                {/* 1. Nút Nhận Việc (Chỉ hiện khi PENDING) */}
                {issue.status === 'PENDING' && (
                    <Button size="sm" icon={Play} onClick={() => handleAccept(issue.id)}>Nhận</Button>
                )}

                {/* 2. Nút Hoàn thành sửa (Chỉ hiện khi PROCESSING và user là người nhận việc hoặc admin) */}
                {issue.status === 'PROCESSING' && (user?.id === issue.assignee?.id || user?.role?.name === 'SUPER_ADMIN') && (
                    <Button size="sm" variant="outline" icon={Wrench} onClick={() => {
                        setSelectedIssueId(issue.id);
                        setWorkflowType("FIX");
                    }}>Xong sửa</Button>
                )}

                {/* 3. Nút Giao hàng (Chỉ hiện khi DELIVERING) */}
                {issue.status === 'DELIVERING' && (user?.id === issue.assignee?.id || user?.role?.name === 'SUPER_ADMIN') && (
                    <Button 
                        size="sm" 
                        className="bg-orange-500 hover:bg-orange-600 text-white border-orange-600 focus:ring-orange-500" 
                        icon={Truck} 
                        onClick={() => {
                            setSelectedIssueId(issue.id);
                ...
                {(issue.status === 'DONE' || issue.status === 'DELIVERED') && (
                    <span className="text-green-600 flex items-center text-xs font-medium"><CheckCircle size={14} className="mr-1"/> Xong</span>
                )}
            </div>
        )
    }
